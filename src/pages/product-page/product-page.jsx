...
const reviews = [
  ...
  },
  {
    id: '2',
    title: 'IGN',
    name: 'by James Duggan',
    rating: '9/10',
    review:
      'Snipe eel cow shark morwong Black mackerel, lungfish loosejaw. Yellowfin grouper tenuis yellow perch pikeblenny hagfish, sarcastic fringehead, kissing gourami hatchetfish featherfin knifefish sharksucker Red salmon daggertooth pike conger. Beaked salmon, smelt porgy zebrafish anglerfish roach.',
  },
  {
    id: '3',
    title: 'Easy Allies',
    name: 'by Daniel Bloodworth',
    rating: '6/10',
    review:
      'Round herring, bonefish bent-tooth sandfish algae eater threespine stickleback alligatorfish dogfish shark. Cownose ray white marlin, dorab, fingerfish Blacksmelt--striped burrfish pickerel, loosejaw wahoo morwong.',
  },
  {
    id: '4',
    title: 'Steam',
    ...
    rating: '7/10',
    review:
      'Kissing gourami hatchetfish featherfin knifefish sharksucker Red salmon daggertooth pike conger. Beaked salmon, smelt porgy zebrafish anglerfish roach, dorab, fingerfish Blacksmelt--striped burrfish pickerel, loosejaw wahoo morwong.',
  },
  {
    id: '5',
    title: 'Easy gaming',
    name: 'by Sara Johnson',
    rating: '10/10',
    review:
      'Black mackerel whitetip reef shark--Atlantic herring Rainbow trout four-eyed fish, mooneye Pacific salmon. Gray reef shark perch codling bluntnose knifefish loweye catfish whitefish mud cat loach minnow roundhead. Sargassum fish cornetfish tilapia anglerfish; carpsucker poacher frogfish sheepshead.',
  },
];
const socials = [
...
export class ProductPage extends Component {
  ...
  render() {
    const {
      game: { loading, data, error, errorDetails },
      addItemToCart,
      history,
    } = this.props;
    const { reviews, socials, inCart } = this.state;

    if (errorDetails && errorDetails.message === '404') {
      ...
      <div className="product">
        ...
        <Container>
          ...
          <BuyProduct
            ...
            onCartClick={() => addItemToCart({ id, image, name, price })}
            onButtonClick={() => {
              ...
              <div className="product-ratings">
                {ratings.map(({ id, title, percent }) => (
                  <div className="product-rating" key={id}>
                    <ProgressItem
                      percents={percent}
                      title={title.replace(/^\w/, (l) => l.toUpperCase())}
                    />
                  </div>
                ))}
              </div>
...
export const ProductTableItem = ({ title, property }) => (
  <div className="product-table-item">
    <Typography component="h5" className="text-muted">
      {typeof property === 'object' && property.length > 1
        ? `${title}s`
        : title}
    </Typography>
    <Typography component="h6" className="mb-0">
      {typeof property === 'object' ? property.join(', ') : property}
    </Typography>
  </div>
);
